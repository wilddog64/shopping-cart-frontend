...
### Changed
- `.github/dependabot.yml` — defer breaking/CI-blocking bumps via `ignore` rules (each needs a dedicated migration; minor/patch still flow). **npm: all majors** deferred via `dependency-name: "*"` (`version-update:semver-major`) — a per-package list (`typescript`, `react-oidc-context`, `tailwindcss`, `@typescript-eslint/*`, `eslint`) proved too narrow when a weekly run opened nine framework majors it didn't cover (React 19, react-router-dom 7, zustand 5, oidc-client-ts 3, tailwind-merge 3, lucide-react 1.x, jsdom 30, eslint-plugin-react-hooks 7 — PRs #74–#82); plus `eslint-plugin-react-refresh` minor (0.5.x requires eslint 9). docker: `node` major (stay on LTS 22; nginx minor still flows). github-actions: all majors (review each manually; minor/patch incl. security still flow). Clears the CI-failing/behind major PRs from the backlog until those upgrades are scheduled.
- Upgrade Prettier `^3.1.1` → `^3.9.6` and reformat `src/components/layout/ProtectedRoute.tsx` + `src/types/index.ts` (3.9 collapses short union types onto one line). Pinning the newer formatter and pre-formatting these two files clears the recurring `npm-minor-patch` Dependabot group Lint failure, where the group's Prettier bump reformatted files that were clean under the old version. Issue: `docs/issues/2026-08-02-npm-minor-patch-group-lint-prettier-drift.md`.
