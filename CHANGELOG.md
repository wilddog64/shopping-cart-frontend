...
- `nginx.conf` — fix `proxy_pass` for `/api/cart` to rewrite to `http://basket:8080/api/v1/cart` (backend Gin group routes are under `/api/v1`; missing the version prefix caused all add-to-cart requests to 404)
