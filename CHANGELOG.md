...
- `.github/workflows/ci.yml` (deploy step): reuse the existing shared `PACKAGES_TOKEN` for the deploy PR flow instead of a new `CI_DEPLOY_PAT` secret. `PACKAGES_TOKEN` already exists in this repo and is used across the backend repos for manifest-bump git pushes, so it carries the needed `repo` scope (Contents + Pull requests) — no new PAT to create or track. It does not bypass branch protection; the deploy PR still passes all required checks before auto-merge. Spec: `docs/plans/frontend-deploy-reuse-packages-token.md`.

### Fixed
- `.github/workflows/ci.yml` (deploy step): the `publish` job no longer pushes the manifest bump directly to protected `main` (which failed with GH006). It now commits the `k8s/base/kustomization.yaml` image-tag bump to a `ci/frontend-image-sha-<sha>` branch, opens a PR with a `CI_DEPLOY_PAT`-authenticated `gh`, and enables auto-merge (`--squash --delete-branch`) so it lands once required checks pass. Adds `pull-requests: write` to the job and a loop guard (`!startsWith(github.event.head_commit.message, 'ci(deploy):')`) so the resulting deploy commit does not re-trigger the pipeline. Requires the `CI_DEPLOY_PAT` repo secret (Contents R/W + Pull requests R/W). Spec: `docs/plans/frontend-deploy-gh006-pr-flow.md`.
