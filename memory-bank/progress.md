...
- [x] **Frontend deploy pipeline hardening** — `077c154` applies the three exact workflow blocks: `ci(deploy)` loop guard, `pull-requests: write`, and PAT-backed auto-merged manifest PR flow. `actionlint` passed; pushed to `fix/frontend-deploy-pipeline`. **PR #69 MERGED** `efd1632` (2026-08-03); CI green; Copilot 2 findings addressed in `6270b6e` (merge deploy PR by URL; spec scope wording) + issue doc `e897716`; threads resolved; `enforce_admins` restored; feature branch deleted. **⚠️ Runtime prereq NOT yet done — the deploy step will fail until the `CI_DEPLOY_PAT` repo secret is created. Fine-grained PAT scoped to `shopping-cart-frontend`, Contents R/W + Pull requests R/W ONLY (no admin, no bypass — the deploy PR passes the 6 required checks and `required_approving_review_count` is 0, so `gh pr merge --auto` lands it through the normal gate). The PAT is required only so the deploy PR triggers `pull_request` CI (a PR opened with the default `GITHUB_TOKEN` does not trigger workflows).**
