...
- [x] **Guest cart persisted across browsers** — `27acd07` captures and reuses `X-Cart-Token`, calls basket `/cart/merge` after login, and clears the token after success. This fixes Safari-only cart badges not appearing in a fresh Chrome session. Issue: `docs/issues/2026-08-04-guest-cart-not-merged-after-login.md`; `npx tsc --noEmit`, lint, and Vitest (26/26) passed.
...
- [x] **Deploy token = reuse `PACKAGES_TOKEN` (no new PAT)** — deploy step `GH_TOKEN` swapped from the never-created `CI_DEPLOY_PAT` to the existing shared `PACKAGES_TOKEN` (already in this repo; backend repos use it for manifest-bump git pushes → proven `repo` scope). No new PAT to create/track. Does not bypass branch protection — deploy PR still passes all required checks before auto-merge. Spec: `docs/plans/frontend-deploy-reuse-packages-token.md`. **PR #70 MERGED** `89ec11e` (2026-08-03); CI green; Copilot 1 finding (stale `CI_DEPLOY_PAT` in CHANGELOG) fixed `114c67a` + issue doc `2026-08-02-copilot-pr70-review-findings.md`; thread resolved; `enforce_admins` restored. **Deploy pipeline is now runtime-ready (no missing secret).** Deploy pipeline then proved itself end-to-end: the CI deploy step auto-opened deploy PRs (#71, #85, …) via `PACKAGES_TOKEN` and auto-merged them.
- [x] **Dependabot backlog cleanup (2026-08-03)** — original 4 open PRs: #64 (defer breaking majors) rebased+**MERGED** `de193a1`; #34 (nginx 1.27→1.31 minor) rebased+**MERGED** `71c8c39`; #37 (node 22→26 major) **CLOSED** (deferred); #68 (npm-minor-patch group) superseded. A weekly run then opened 9 framework majors the #64 per-package allowlist missed (React 19, router 7, zustand 5, oidc-client-ts 3, etc., #74–#82). Fix: PR #84 **MERGED** `dc22f30` broadens the npm ignore to `dependency-name: "*"` semver-major (spec `docs/plans/dependabot-ignore-npm-majors.md`); #74–#82 closed via `@dependabot close`. Stale deploy PR #83 closed (would regress image tag). Prettier drift root-caused: the group's 3.7→3.9 bump reformats 2 files clean on main (`ProtectedRoute.tsx`, `types/index.ts`); PR #87 **MERGED** `7e72de5` pins Prettier `^3.9.6` on main + pre-formats both (3.7.4 rejects the 3.9 format, so the bump was required) — recurring group Lint red cleared; recreated group #86 rebased onto fixed main. Issue `docs/issues/2026-08-02-npm-minor-patch-group-lint-prettier-drift.md`.
